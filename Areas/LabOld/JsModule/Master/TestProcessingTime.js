$(document).ready(function () {
    ...
    $('#tblTestDetails thead').on('keyup', 'input[type=text]', function () {
        var time = $(this).val();
        $('#tblTestDetails tbody tr').find('td:eq(4) input').val(time);
            ...
            if (Object.keys(data.ResultSet).length > 0) {
                if (Object.keys(data.ResultSet.Table).length > 0) {
                    $.each(data.ResultSet.Table, function (key, val) {
                        ...
                        tbody += "<td><input type='text' style='color: black; background: #dfe8f1;' class='text-right' value='" + val.max_time + "'/></td>";
                        tbody += "</tr>";
                    });
    ...
    if (confirm('Are you sure to Insert Processing Time')) {
        ...
        $('#tblTestDetails tbody tr').each(function () {
            objBO.push({
                ...
                'SampleCode': '',
                //'ProcessingTime': $(this).find('td:eq(4)').val(),
                'ProcessingTime': $(this).find('td:eq(4) input').val(),
