...
$(document).ready(function () {
    ...
    $('#tblPaymentDetails tbody').on('keyup', 'input[type=text]', function () {
        var IsAdvanceCheck = $(this).hasClass('advanceCheck');
        if (IsAdvanceCheck) {
            ValidateAdvanceAmt($(this))
        }
        ...
        var advance = parseFloat($('#tblPaymentDetails tbody').find('tr:eq(4)').find('td:eq(1)').find('input[type=text]').val());
        var total = cash + cheque + cc + ntfs + advance;
        if (total > netAmount) {
            ...
            $('#tblPaymentDetails tbody').find('tr:eq(0),tr:eq(1),tr:eq(2),tr:eq(3),tr:eq(4)').find('td:eq(1)').find('input[type=text]').removeAttr('style');
    ...
    $('.TimeFrame').on('click', 'span.TimeSlot:not([disabled])', function () {
        ...
        var time = $(this).text().substring(0, 17) + "-" + this.id;
    ...
    $('span.TimeSlot2').on('click', function () {
        //var time = $(this).not('span').text();
        var time = $(this).text().substring(0, 17) + "-" + this.id;
        var date = $('#txtTimeSlotsByDate').val();
        $('#txtTime').val(time);
...
function GetAdvanceForBooking() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var Balance = data.ResultSet.Table[0].Balance;
            $('#tblPaymentDetails tbody').find('tr:eq(4)').find('td:eq(3)').text('OPD Advance : ' + Balance + ' Rs.').attr('data-amt', Balance);
        },
        error: function (response) {
            alert('Server Error...!');
        }
...
function ValidateAdvanceAmt(elem) {
    var amt = parseFloat($(elem).val());
    var advanceAmt = parseFloat($(elem).closest('tr').find('td:eq(3)').data('amt'));
    if (amt > advanceAmt) {
        $(elem).val('');
        ...
        success: function (data) {
            ...
            if (Object.keys(data.ResultSet).length > 0) {
                if (Object.keys(data.ResultSet.Table1).length > 0) {
                    $.each(data.ResultSet.Table1, function (key, val) {
                        var time = val.Final_InShift + '-' + val.Final_outShift + '-' + val.TokenNo;
                        $('#txtTime').val(time);
                    });
                }
            }
            if (Object.keys(data.ResultSet).length > 0) {
                if (Object.keys(data.ResultSet.Table2).length > 0) {
                    $.each(data.ResultSet.Table2, function (key, val) {
                        $('.paymentSection span.tokenNo span:last').text((eval(val.TokenNo) < 10 ? '0' + val.TokenNo : val.TokenNo));
