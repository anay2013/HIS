...
namespace MediSoftTech_HIS.Areas.Lab.Repository
{
    public class GenReportIPD
    {
        ...
        public FileResult IPDPrintLabReport(string IPDNo)
        {
            string visitNo;
            string TestIds = "";
            ReportPrintingInfo objM = new ReportPrintingInfo();
            objM.LabCode = "-";
            objM.PanelId = "-";
            objM.DoctorId = "-";
            objM.VisitNo = IPDNo;
            objM.TestCategory = "-";
            objM.TestIds = TestIds;
            objM.from = "1900/01/01";
            objM.to = "1900/01/01";
            objM.Prm1 = "-";
            objM.Prm2 = "-";
            objM.Logic = "IPDGetALLVisits";
            dataSet  dsResultAll = APIProxy.CallWebApiMethod("Lab/Lab_ReportPrintingQueries", objM);
            PdfDocument repDocumentAll = new PdfDocument();
            repDocumentAll.SerialNumber = "PXVUbG1Z-W3FUX09c-T0QMCBMN-HQwdDh0M-HQ4MEwwP-EwQEBAQ=";
            foreach (DataRow dr in dsResultAll.ResultSet.Tables[0].Rows)
            {
                pgCounterList = new List<ipPageCounter>();
                visitNo = dr["VisitNo"].ToString();
                TestIds = dr["TestIds"].ToString();
                ...
                if(dsResult.ResultSet.Tables.Count > 6)
                {
                    ...
                    {
                        foreach (DataRow dr2 in dsResult.ResultSet.Tables[6].Rows)
                        {
                            string AddedReportPath = dr2["AddedReportPath"].ToString();
                ...
                PdfDocument T = PdfDocument.FromStream(new MemoryStream(repDocument.WriteToMemory()));
                repDocumentAll.AddDocument(T);
            }
            byte[] pdfdata = repDocumentAll.WriteToMemory();
