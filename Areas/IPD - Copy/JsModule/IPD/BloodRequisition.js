...
function PrintReport(IndentNo) {
    var url = config.documentServerUrl + "/IPD/Print/BloodRequisitionForm?IndentNo=" + IndentNo;
...
function Insert() {
    ...
    bloodRequisitionInfo.IPOPType = 'IPD';
    ...
    bloodRequisitionInfo.AboRH = $('#ddlABORh option:selected').val();
...
function doctorSign(val) {
    $('#imgDoctorSign').prop('src', config.documentServerUrl + val);
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (Object.keys(data.ResultSet).length) {
                ...
                if (Object.keys(data.ResultSet.Table1).length) {
                    $('#ddlABORh').append($('<option></option>').val('-').html('-')).select2();
                    $.each(data.ResultSet.Table1, function (key, val) {
                        $('#ddlABORh').append($('<option></option>').val(val.BloodGroupId).html(val.BloodGroupAllotted));
                    });
                }
            }
        },
