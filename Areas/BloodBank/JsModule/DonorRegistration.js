...
$(document).ready(function () {
    ...
    $('#start-camera').on('click', async function () {
        ...
        $('#liveCamera label').text('Camera is on..');
    });
                ...
                if (Object.keys(data.ResultSet.Table).length) {
                    ...
                    if (data.ResultSet.Table.length > 0) {
                        $.each(data.ResultSet.Table, function (key, val) {
                            ...
                            tbody += "<td><button onclick=FillFormData(this) class='btn btn-success btn-xs search-tbl-btn'>Select</button></td>";
                            tbody += "</tr>";
                            count++;
                        });
...
function FillFormData(elem) {
    var info = JSON.parse($(elem).closest('tr').find('td:eq(0)').text());
    $('#txtDonorId').val(info.donor_id);
    $('#ddlTitle option').each(function () {
        if ($(this).text() == info.Title)
            $(this).parents('select').prop('selectedIndex', '' + $(this).index() + '').change();
    });
    $('#txtFirstName').val(info.Dfirstname);
    $('#txtLastName').val(info.Dlastname);
    $('#txtDOB').val(info.dtBirth);
    $('#txtAge').val(info.age);
    $('input[value=' + info.Gender + ']').prop('checked', true);
    $('#txtRelativeName').val(info.relative_Name);
    $('#ddlRelation').val(info.Relation).change();
    $('#ddlBloodGroup option').each(function () {
        if ($(this).text() == info.donor_BloodGroup)
            $(this).parents('select').prop('selectedIndex', '' + $(this).index() + '').change();
    });
    $('#txtAddress').val(info.donor_Address);
    $('#ddlCountry option').each(function () {
        if ($(this).text() == info.Country)
            $(this).parents('select').prop('selectedIndex', '' + $(this).index() + '').change();
    });
    $('#ddlCity option').each(function () {
        if ($(this).text() == info.City)
            $(this).parents('select').prop('selectedIndex', '' + $(this).index() + '').change();
    });
    $('#txtPinCode').val(info.PinCode);
    $('#txtContactNo').val(info.contactNo);
    $('#txtAadharNo').val(info.aadharNo);
    $('#txtEmail').val(info.Email);
    $('#modalDonorSearch').modal('hide');
    $('#tblDonorSearchInfo tbody').empty();
    $('#txtSearchValue').val('');
}
