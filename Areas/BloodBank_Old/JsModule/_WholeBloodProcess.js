...
$(document).ready(function () {
    ...
    //    keyboard: false
    //})
    CheckNav = window.location.pathname.split('/');
    ...
    if (CheckNav[CheckNav.length - 1] == 'WholeBloodProcess')
        DonorProcess("Donor:Inprocess");
    else
        DonorProcess("DonorApproval:Inprocess");
    ...
    $('#tblComponentInfo thead').on('change', 'input:checkbox', function () {
        var isCheck = $(this).is(':checked');
        $('#tblComponentInfo tbody tr').find('#txtPackSize').removeClass('error');
        if (isCheck)
    ...
    $('#tblComponentInfo tbody').on('change', 'input:checkbox', function () {
        ...
        $(this).closest('tr').find('td:eq(6)').find('#txtPackSize').removeClass('error');
        if ($(this).is(':checked')) {
            ...
            $('#tblComponentInfo tbody tr').find('input[type=checkbox]:checked').each(function () {
                if ($(this).closest('tr').find('td:eq(6)').find('#txtPackSize').val() == '')
                    $(this).closest('tr').find('td:eq(6)').find('#txtPackSize').addClass('error');
                else
                    $(this).closest('tr').find('td:eq(6)').find('#txtPackSize').removeClass('error');
            });
        }
    ...
    $('#tblDonorProcessInfo tbody').on('click', '#IdWisePrint', function () {
        debugger
            ...
            if (Object.keys(data.ResultSet).length) {
                if (Object.keys(data.ResultSet.Table).length) {
                    ...
                    $.each(data.ResultSet.Table, function (key, val) {
                        ...
                        DoonorInfo += "<td><button class='btn btn-warning btn-xs' id='IdWisePrint'>Print</button></td>";
                        DoonorInfo += "</tr>";
            ...
            if (Object.keys(data.ResultSet).length) {
                if (Object.keys(data.ResultSet.Table).length) {
                    ...
                    $.each(data.ResultSet.Table, function (key, val) {
                        ...
                        tbody += "<td><input type='text' id='txtPackSize' class='form-control' style='height: 24px;padding: 4px;text-align:right'/></td>";
        ...
        $('#tblComponentInfo tbody tr').each(function () {
            if ($(this).find('input[type=checkbox]').is(':checked')) {
                ...
                componentInfo.push({
                    ...
                    'PackSize': $(this).closest('tr').find('td:eq(6)').find('#txtPackSize').val() + ' ml',
...
function GetInfoForApproval() {
    ...
    $('.tblApproveInfo #lblDonationType').text('');
            ...
            if (Object.keys(data.ResultSet).length) {
                ...
                if (Object.keys(data.ResultSet.Table2).length) {
                    ...
                    $.each(data.ResultSet.Table2, function (key, val) {
                        $('#lblDonationType').text(val.DonationType);
                        if (_IsDiscarded == 'N') {
                            if (val.Result != null) {
...
function ApproveDonation(elem) {
    if (confirm('Are you sure to Approve')) {
        ...
        objBO.Prm2 = '-';
        objBO.login_id = Active.userId;
        objBO.Logic = ($('#lblDonationType').text() == 'SDP') ? 'SDP-Approve' :'Approve';
        $.ajax({
            method: "POST",
            url: url,
            data: JSON.stringify(objBO),
...
function PrintInfo(vistid) {
    var url = config.documentServerUrl + "/BloodBank/PrintBlood/BloodBankPrint?visit_id=" + visitid;
