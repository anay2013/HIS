    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ResultSet.Table.length > 0) {
                $("ddlCart").empty();
                $("#ddlCart").append($("<option></option>").val("0").html("Please Select")).select2();
...
function GetItem() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ResultSet.Table.length > 0) {
                $("ddlItem").empty();
                $("#ddlItem").append($("<option></option>").val("0").html("Please Select")).select2();
                $("#ddlItem").append($("<option></option>").val("ALL").html("ALL"));
                $.each(data.ResultSet.Table, function (key, value) {
                    $("#ddlItem").append($("<option></option>").val(value.item_id).html(value.item_name));
                });
            }
        },
        error: function (response) {
