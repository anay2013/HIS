...
function LedgerByLedgerId(ledgerid) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ResultSet.Table.length > 0) {
                $.each(data.ResultSet.Table, function (key, val) {
                    $('input[name=TallySync]').prop('checked', (val.isTallySync == 'Y') ? true : false);
                    $('input[name=linkCode]').prop('checked', (val.LinkCode == 'Y') ? true : false);
...
function CreateLedgers() {
    if (Validation()) {
        ...
        objBO.IsTallySync = ($('input[name=TallySync]').is(':checked')) ? 'Y' : 'N';
        objBO.IsLinkCode = ($('input[name=linkCode]').is(':checked')) ? 'Y' : 'N';
...
function Clear() {
    ...
    $('#txtOpeningAmt').val('');
    $('#txtPartyAddress').val('');
    $('input[name=TallySync]').prop('checked', false);
    $('input[name=linkCode]').prop('checked', false);
