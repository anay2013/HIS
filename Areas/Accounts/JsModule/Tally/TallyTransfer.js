$(document).ready(function () {
    ...
    $('#tblTallyVoucher thead').on('click', 'button[title=send]', function () {
        $('#tblTallyVoucher tbody tr').each(function () {
            debugger
            var $row = $(this);
            var $sendButton = $row.find('button[title=send]');
            $sendButton.click();
        });
    ...
    $('#tblTallyVoucher tbody').on('click', 'button[title=send]', function () {
        ...
        var VoucherNo = $row.find('td:eq(4)').text();
        var SyncStatus = $row.find('td:eq(8)').text();
        VoucherForSync(VoucherNo, $row);
    });
...
function VoucherForSync(VoucherNo, elem) {
    $("#tblTallyVoucher thead").find("tr").find("th button[title='send']").prop("disabled", true);
    var count = 0;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data == 'Successfully Saved') {
                ...
                $(elem).find('button[title="send"]').prop('disabled', true);
            }
            else {
                $(elem).find('td:eq(9)').find('button[title=status]').removeAttr('class');
