...
namespace MediSoftTech_HIS.Areas.IPD.Controllers
{
    public class PrintController : Controller
    {
        ...
        public FileResult PrintEstimateForm(string UHIDNO, string estNO)
        {
            PdfGenerator pdfConverter = new PdfGenerator();
            ...
            objBO.Logic = "PrintEstimateEntry";
            HISWebApi.Models.dataSet dsResult = APIProxy.CallWebApiMethod("IPDBilling/IPD_EstimateTemplateQueries", objBO);
            DataSet ds = dsResult.ResultSet;
            string _result = string.Empty;
            StringBuilder b = new StringBuilder();
            StringBuilder h = new StringBuilder();
            ...
            if (ds.Tables.Count > 0 && ds.Tables[2].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[2].Rows)
                {
                    ...
                    toWhom = dr["BillingType"].ToString();
                    Amount = dr["Panel"].ToString();
                    content = dr["estContent"].ToString();
                    foreach (var var1 in dr["VarList"].ToString().Split(','))
                    {
