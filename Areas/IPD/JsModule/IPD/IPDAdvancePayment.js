        ...
        success: function (data) {
            if (data.ResultSet.Table.length > 0) {
                $.each(data.ResultSet.Table, function (key, val) {
                    ...
                    $('#txtRoomFullName').text(val.roomFullName);
                    $('#txtAgeInfo').text(val.ageInfo);
                    $('#txtDoctor').text(val.DoctorName);
                    $('#txtAdmitDate').text(val.AdmitDate);
                });
            }
        },
...
function ReceiptInfo() {
    ...
    objBO.Logic = 'fin_ReceiptInfo:IPD';
    $.ajax({
        method: "POST",
        url: url,
        data: JSON.stringify(objBO),
