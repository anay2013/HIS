        ...
        success: function (data) {
            if (Object.keys(data.ResultSet).length > 0) {
                ...
                if (Object.keys(data.ResultSet.Table7).length) {
                    $('#ddlBillingCategory').empty().append($('<option>Select</option>')).trigger('change.select2');
        ...
        success: function (data) {
            ...
            if (Object.keys(data.ResultSet).length) {
                if (Object.keys(data.ResultSet.Table1).length) {
                    $('#ddlRoomBed').empty().append($('<option>Select</option>')).trigger('change.select2');
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.includes('Success')) {
                        ...
                        _IPOPNo = data.split('|')[1];
                        IPDRegisteredInfo(_IPOPNo);
                        PrintReceipt();
                        Clear();
...
function ValidateRegistration() {
    ...
    var RoomBedId = $('#txtRoomBedId').val();
    var RquestRoom = $('#ddlRquestRoom option:selected').text();
    ...
    if (RoomBedId == '') {
        $('#txtRoomBedId').css('border-color', 'red').focus();
        alert('Please Provide Room Bed Info..');
        return false;
    }
    else {
        $('#txtRoomBedId').removeAttr('style');
    }
    //if (RquestRoom == 'Select') {
    //    $('span.selection').find('span[aria-labelledby=select2-ddlRquestRoom-container]').css('border-color', 'red').focus();
    //    alert('Please Select Rquest Room..');
    //    return false;
    //}
    //else {
    //    $('span.selection').find('span[aria-labelledby=select2-ddlRquestRoom-container]').removeAttr('style');
    //}
    if (AttendantName == '') {
