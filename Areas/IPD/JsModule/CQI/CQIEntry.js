$(document).ready(function () {
    ...
    $('.navigation ul li').on('click', function () {
        $('.navigation ul li').removeClass('ac-success');
        $(this).addClass('ac-success');
        OpenBody($(this).data('form'))        
    });
});
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.ResultSet.Table, function (key, val) {
                ...
                html += "<button data-ipd=" + val.IPDNo + " data-name='" + val.PatientName + "'  class='btn btn-success btn-xs pull-right' onclick=GetPatientInfo(this)><i class='fa fa-eye'>&nbsp;</i>Open</button>";
                html += "</span>";
                html += "</td>";
                html += "</tr>";
                html += "</tr>";
...
function GetPatientInfo(elem) {
    ...
    var user = "<b>IPD No : </b>" + $(elem).data('ipd') + ', <b>Patient Name : </b>' + $(elem).data('name');
    $('.navigation label:eq(0)').html(user);
