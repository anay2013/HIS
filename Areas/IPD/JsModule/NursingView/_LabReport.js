...
function ReportInfo() {
    if ($('#ddstatus option:selected').val() == "Provisional") {
        ReportInfoProvisional();
    }
    else if ($('#ddstatus option:selected').val() == "ALL") {
...
function ReportList(data, Logic) {
    ...
    $.each(data.ResultSet.Table, function (key, val) {
        if (visitNo != val.VisitNo) {
            ...
            if ($('#ddstatus option:selected').val() == "ALL") {
            ...
            else if ($('#ddstatus option:selected').val() == "Provisional") {
                tbody += "<td><input type='checkbox' name='pgroup'/>&nbsp;Select All&nbsp;&nbsp;<button onclick=PrintProvisional(this) class='btn btn-warning btnPrint btn-xs'>Print Provisional</button></td>";
            }
            tbody += "</tr>";
            visitNo = val.VisitNo;
        }
...
function ReportInfoProvisional() {
    var Logic = "IPD:TestWiseReport";
    $("#tblReport tbody").empty();
    var url = config.baseUrl + "/api/Lab/Lab_ReportProvisionalQueries";
    var objBO = {};
    objBO.PanelId = '-';
    objBO.DoctorId = '-';
    objBO.VisitNo = $('#tblAdviceHeader tbody tr:eq(0)').find('td:eq(9)').text()
...
function PrintProvisional(elem) {
    var visitNo = $(elem).closest('tr').find('td:eq(1)').text();
    var SubCat = 'ALL';
    var TestIds = [];
    TestIds = [];
    ...
    var Logic = 'PrintProvisionalReport';
    var url = config.rootUrl + "/Lab/print/PrintLabReport?visitNo=" + visitNo + "&SubCat=" + SubCat + "&TestIds=" + TestIds.join() + "&Source=In-House&Logic=" + Logic;
    window.open(url, '_blank');
}
