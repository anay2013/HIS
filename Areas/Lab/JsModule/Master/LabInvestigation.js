$(document).ready(function () {
    ...
    $('#get-started').on('click', function () {
        ...
        if ($this.hasClass('active')) {
            ...
            return $xyz.removeClass('active');
        } else {
            $this.addClass('active');
            return $xyz.addClass('active');
        }
    });
    $('#get-started').on('click', function () {
        var $xyz, $this;
        ...
        if ($this.hasClass('open')) {
            ...
            return $xyz.removeClass('open');
        } else {
            $this.addClass('open');
            return $xyz.addClass('open');
        }
    });
    ...
    $('#tblInvestgationDetails tbody').on('click', '.InvestgationDetails', function () {
        var Investgationid = $(this).closest('tr').find('td:eq(0)').text();
        var textcode = $(this).closest('tr').find('td:eq(1)').text();
        var textName = $(this).closest('tr').find('td:eq(2)').text();
        var subcat = $(this).closest('tr').find('td:eq(3)').text();
        $("#txtExtCode").val(textcode);
        $("#txtExtCode").prop('disabled', true);
        $("#txtInvestCode").val(Investgationid);
        $("#txtInvestCode").prop('disabled', true);
        $("#txtInvestName").val(textName);
        $("#ddlSubCategory option:selected").text(subcat);
        ...
        success: function (data) {
            ...
            if (Object.keys(data.ResultSet).length > 0) {
                ...
                if (Object.keys(data.ResultSet.Table).length > 0) {
                    ...
                    // $("#divInvestCode").show();
                    $("#txtExtCode").val(data.ResultSet.Table[0].extTestCode);
                    $("#txtExtCode").prop('disabled', true);
...
function SearchByCode() {
    ...
    var url = config.baseUrl + "/api/temporary/GetLISTestInfo";
    var objBO = {};
    objBO.Prm1 = $("#txtSearchCode").val();
    objBO.Logic = 'ByTestCode';
    $.ajax({
        method: "POST",
        url: url,
        data: JSON.stringify(objBO),
            ...
            if (Object.keys(data.ResultSet).length) {
                if (Object.keys(data.ResultSet.Table).length) {
                    ...
                    $.each(data.ResultSet.Table, function (key, val) {
                        ...
                        tbody += '<td><button class="btn btn-success InvestgationDetails"><i class="fa fa-arrow-right"></i></button></td>';
                        tbody += "</tr>";
