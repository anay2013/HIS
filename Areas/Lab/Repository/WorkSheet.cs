...
namespace MediSoftTech_HIS.Areas.Lab.Repository
{
    public class WorkSheet
    {
        ...
        public FileResult PrintWorkSheet(string visitNo, string SubCat,string TestIds,string Logic)
        {
            ...
            }).ToList().OrderBy(y =>y.SeqNo).GroupBy(x => new { x.SubCatName});
        ...
        private string GetBodyHTML(string DepartmentName, DataSet ds,string IsNABL)
        {
            ...
            var ObsTest = ds.Tables[1].AsEnumerable().Where(z => z.Field<string>("SubCatName") == DepartmentName).Select(y => new
            {
